...
chemin_model = 'modeles/Xception_model_75_7.h5'

model_with_pseudo_label = load_model(chemin_model)

model_with_pseudo_label.compile(optimizer=SGD(learning_rate=5e-04), loss='binary_crossentropy',  metrics=['accuracy', Recall(name='recall'), Precision(name='precision')])



chemin_model = 'modeles/InceptionResNetV2_model_75_8.h5'

model_with_pseudo_label_inceptionResNetV2 = build_model_InceptionResNetV2_opt(Adam(learning_rate=0.005))
model_with_pseudo_label_inceptionResNetV2.load_weights(chemin_model)
...
previous_val_performance = evaluate_model(model_with_pseudo_label, X_val, y_val)

print(previous_val_performance)



iteration = 1

while len(X_non_labelised) >= 0 and iteration <= 16:
    ...
    if(len(all_X_train_with_pseudo_label) > 2000):
        batch = 128
    elif(len(all_X_train_with_pseudo_label) > 600):
        batch = 64
    else:
        batch = 32
    ...
    model_with_pseudo_label = build_model_xception_opt(SGD(learning_rate=5e-04))
      
    model_with_pseudo_label.compile(optimizer=SGD(learning_rate=5e-04, momentum=0.9), loss='binary_crossentropy', metrics=['accuracy', Recall(name='recall'), Precision(name='precision')])
    ...
    print(f"Affiche a chaque entrainement comme backup iteration {iteration} pour Xception")
